...
const AdminLoginForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      ...
      if (user.role === 'admin' || user.role === 'super_admin') {
        // Don't redirect here if we're showing the success modal
        // The modal close handler will redirect
        if (!showSuccessModal) {
          router.push('/admin');
        }
      } else {
        // If user is not an admin, logout and redirect to login
        useAuthStore.getState().logout();
        router.push('/oko-admin');
      }
    }
  }, [isAuthenticated, user, router, showSuccessModal]);
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (currentUser && (currentUser.role === 'admin' || currentUser.role === 'super_admin')) {
          ...
          useAuthStore.getState().logout();
          useAuthStore.getState().setError('Access denied. Admin credentials required.');
        }
      }, 100);
  ...
  const handleSuccessModalClose = () => {
    ...
    // Navigate to admin dashboard after modal closes
    router.push('/admin');
  };
    ...
    <>
      ...
      <div className="p-4 sm:p-6 lg:p-8 bg-white/80 rounded-lg shadow-lg max-w-md mx-auto lg:mx-0">
        ...
        <div className="mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-semibold text-mainGreen mb-2">Admin Login</h1>
          <p className="text-gray-600 text-xs sm:text-sm">Access the admin dashboard to manage the platform.</p>
        </div>
